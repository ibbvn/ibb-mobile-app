...
import fieldDefinitions from '../assets/hanoi.json';
...
const HanoiScreen = ({ user, navigation, route }) => {
  const { editMode = false, batchData = null, fileName = null, beerType = 'hanoi' } = route.params || {};
  ...
  useEffect(() => {
    ...
    if (editMode && batchData) {
      ...
      console.log('📝 Edit mode: Loading existing batch data for Hanoi');
      ...
      console.log('✨ Create mode: Initializing new Hanoi batch');
      const initialData = {
  ...
  const uploadImageToServer = async (imageUri, fieldId) => {
    try {
      ...
      // Tên file: Hanoi_Tank[X]_Batch[Y]_Field[Z]_[timestamp].jpg
      const fileName = `Hanoi_Tank${tankNumber}_Batch${batchNumber}_${fieldId}_${timestamp}.jpg`;
      ...
      formData.append('folder', 'Chebien/Hanoi');
      ...
      formData.append('beerType', 'hanoi');
      ...
      if (response.ok) {
        const result = await response.json();
        console.log('Hanoi Image uploaded successfully:', result);
        ...
        Alert.alert('Thành công', `Ảnh Bia Hà Nội đã được lưu thành công\nFile: ${result.fileName}`);
      } else {
      ...
      console.error('Error uploading Hanoi image:', error);
      Alert.alert(
        'Cảnh báo', 
  ...
  const saveBatch = async (batchData) => {
    try {
      ...
      Alert.alert('Thành công', 'Phiếu nấu Bia Hà Nội đã được lưu thành công!');
      
    } catch (error) {
      console.error('Error saving Hanoi batch:', error);
  ...
  const saveToLocalFile = async (data) => {
    try {
      ...
      if (editMode && currentFileName) {
        ...
        console.log(`📝 Edit mode: Updating existing Hanoi file ${fileName}`);
      } else {
        // Create mode: tạo file mới với prefix hanoi
        const now = new Date();
        const ngay = now.toISOString().split('T')[0]; // YYYY-MM-DD
        const gio = now.toTimeString().split(' ')[0].replace(/:/g, ''); // HHMMSS
        const meSo = data.field_002 || data.me_so || activeBatch;
        fileName = `${ngay}__hanoi_me${meSo}__${gio}.json`;
        console.log(`✨ Create mode: Creating new Hanoi file ${fileName}`);
      }
      ...
      console.log(`✅ Saved Hanoi to local file: ${fileName}`);
      setCurrentFileName(fileName); // Update current file name
      return fileName;
      
    } catch (error) {
      console.error('❌ Error saving Hanoi to local file:', error);
  ...
  const syncToServer = async (data) => {
    try {
      ...
      if (response.ok) {
        const result = await response.json();
        console.log('✅ Synced Hanoi to server:', result.message);
      } else {
        console.warn('⚠️ Hanoi Server sync failed, but local file saved');
      }
    } catch (error) {
      console.warn('⚠️ Hanoi Server sync failed:', error.message, '(Local file still saved)');
        ...
        <View key={fid} style={{ marginBottom: 10 }}>
          ...
          >
            <Text style={{ color: 'white', fontWeight: 'bold', fontSize: 16 }}>
              📸 Chụp ảnh Hà Nội {field.label.includes('Plato') ? 'Plato' : ''}
            </Text>
          </TouchableOpacity>
            ...
            <View>
              <Image 
                ...
                style={{ 
                  ...
                  borderRadius: 10,
                  borderWidth: 2,
                  borderColor: serverImage ? '#4CAF50' : '#dc3545'
                }} 
              />
              
              {/* Hiển thị trạng thái upload */}
              <View style={{
                backgroundColor: serverImage ? '#4CAF50' : '#dc3545',
                padding: 8,
                borderRadius: 5,
                marginTop: 5
              }}>
              ...
              <Text style={{ 
                ...
                Tên file: Hanoi_Tank{batches[activeBatch]?.field_003 || 'X'}_Batch{batches[activeBatch]?.field_002 || activeBatch}_{fid}
              </Text>
            </View>
          )}
        </View>
      ...
      <ScrollView horizontal style={{ backgroundColor: '#eee', flexDirection: 'row', paddingVertical: 4, minHeight: TAB_HEIGHT }}>
        {batchIds.map((batchId) => (
          <View key={batchId} style={{
            ...
            backgroundColor: activeBatch === batchId ? '#dc3545' : '#fff',
        ...
        <TouchableOpacity
          ...
          <Text style={{ color: '#dc3545', fontWeight: 'bold', fontSize: 24 }}>＋</Text>
        </TouchableOpacity>
      </ScrollView>

      <ScrollView style={{ padding: 10 }}>
        <Text style={{ fontSize: 18, fontWeight: 'bold', marginBottom: 10 }}>
          🏯 Bia Hà Nội - {user?.full_name || 'Người dùng'} - Ngày: {nowDate}
        </Text>
        
        {/* Debug info */}
        {editMode && (
          <View style={{ backgroundColor: '#f8d7da', padding: 10, borderRadius: 5, marginBottom: 10 }}>
            <Text style={{ fontSize: 14, color: '#721c24', fontWeight: 'bold' }}>
              📝 Chế độ chỉnh sửa - Bia Hà Nội
            </Text>
            {currentFileName && (
        ...
        <Button title='💾 Lưu phiếu Bia Hà Nội' onPress={handleSubmit} />
