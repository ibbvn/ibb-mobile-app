...
import fieldDefinitions from '../assets/chaihg.json';
import { saveBatchFile } from '../utils/saveBatchFile';
...
const ChaihgScreen = ({ user, navigation, route }) => {
  const { editMode = false, batchData = null, fileName = null, beerType = 'chaihg' } = route.params || {};
  ...
  useEffect(() => {
    ...
    if (editMode && batchData) {
      ...
      console.log('📝 Edit mode: Loading existing batch data for ChaiHG');
      ...
      console.log('✨ Create mode: Initializing new ChaiHG batch');
      const initialData = {
  ...
  const uploadImageToServer = async (imageUri, fieldId) => {
    try {
      ...
      // Tên file: ChaiHG_Tank[X]_Batch[Y]_Field[Z]_[timestamp].jpg
      const fileName = `ChaiHG_Tank${tankNumber}_Batch${batchNumber}_${fieldId}_${timestamp}.jpg`;
      ...
      formData.append('folder', 'Chebien/ChaiHG');
      ...
      formData.append('beerType', 'chaihg');
      ...
      if (response.ok) {
        const result = await response.json();
        console.log('ChaiHG Image uploaded successfully:', result);
        ...
        Alert.alert('Thành công', `Ảnh ChaiHG đã được lưu thành công\nFile: ${result.fileName}`);
      } else {
      ...
      console.error('Error uploading ChaiHG image:', error);
      Alert.alert(
        'Cảnh báo', 
  ...
  const handleSaveBatch = async (batchData) => {
    try {
      ...
      Alert.alert('Thành công', 'Phiếu nấu Bia Chai Hoàng Gia đã được lưu thành công!');
      
    } catch (error) {
      console.error('Error saving ChaiHG batch:', error);
  ...
  try {
    const fileName = await saveBatchFile(data);
    setCurrentFileName(fileName);
    console.log(`✅ Saved ChaiHG to local file: ${fileName}`);
    return fileName;
  } catch (error) {
    console.error('❌ Error saving ChaiHG to local file:', error);
    throw new Error('Không thể lưu file local: ' + error.message);
  }
};
const saveBatch = async (batchData) => {
  try {
    await saveToLocalFile(batchData);
    // Nếu muốn đồng bộ server thì thêm: await syncToServer(batchData);
    // Nếu chỉ lưu local thì dòng trên là đủ
    Alert.alert('Thành công', 'Phiếu đã được lưu thành công!');
  } catch (error) {
    Alert.alert('Lỗi', 'Có lỗi khi lưu phiếu: ' + error.message);
  }
};
  ...
  const syncToServer = async (data) => {
    try {
      ...
      if (response.ok) {
        const result = await response.json();
        console.log('✅ Synced ChaiHG to server:', result.message);
      } else {
        console.warn('⚠️ ChaiHG Server sync failed, but local file saved');
      }
    } catch (error) {
      console.warn('⚠️ ChaiHG Server sync failed:', error.message, '(Local file still saved)');
        ...
        <View key={fid} style={{ marginBottom: 10 }}>
          ...
          >
            <Text style={{ color: 'white', fontWeight: 'bold', fontSize: 16 }}>
              📸 Chụp ảnh ChaiHG {field.label.includes('Plato') ? 'Plato' : ''}
            </Text>
          </TouchableOpacity>
            ...
            <View>
              <Image 
                ...
                style={{ 
                  ...
                  borderRadius: 10,
                  borderWidth: 2,
                  borderColor: serverImage ? '#4CAF50' : '#ffc107'
                }} 
              />
              
              {/* Hiển thị trạng thái upload */}
              <View style={{
                backgroundColor: serverImage ? '#4CAF50' : '#ffc107',
                padding: 8,
                borderRadius: 5,
                marginTop: 5
              }}>
              ...
              <Text style={{ 
                ...
                Tên file: ChaiHG_Tank{batches[activeBatch]?.field_003 || 'X'}_Batch{batches[activeBatch]?.field_002 || activeBatch}_{fid}
              </Text>
            </View>
          )}
        </View>
      ...
      <ScrollView horizontal style={{ backgroundColor: '#eee', flexDirection: 'row', paddingVertical: 4, minHeight: TAB_HEIGHT }}>
        {batchIds.map((batchId) => (
          <View key={batchId} style={{
            ...
            backgroundColor: activeBatch === batchId ? '#ffc107' : '#fff',
        ...
        <TouchableOpacity
          ...
          <Text style={{ color: '#ffc107', fontWeight: 'bold', fontSize: 24 }}>＋</Text>
        </TouchableOpacity>
      </ScrollView>

      <ScrollView style={{ padding: 10 }}>
        <Text style={{ fontSize: 18, fontWeight: 'bold', marginBottom: 10 }}>
          👑 Bia Chai Hoàng Gia - {user?.full_name || 'Người dùng'} - Ngày: {nowDate}
        </Text>
        
        {/* Debug info */}
        {editMode && (
          <View style={{ backgroundColor: '#fff3cd', padding: 10, borderRadius: 5, marginBottom: 10 }}>
            <Text style={{ fontSize: 14, color: '#856404', fontWeight: 'bold' }}>
              📝 Chế độ chỉnh sửa - Bia Chai Hoàng Gia
            </Text>
            {currentFileName && (
        ...
        <Button title='💾 Lưu phiếu Bia Chai Hoàng Gia' onPress={handleSubmit} />
          ...
          <TouchableOpacity
            ...
            onPress={async () => {
              try {
                ...
                Alert.alert('Files in chebien/', files.join('\n') || 'No files found');
              } catch (error) {
